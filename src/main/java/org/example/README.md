...
## TODO / Поради з розвитку
- `GetHandler` все ще використовує захардкожений `clientID` у деяких місцях — бажано повністю перейти на ID з автентифікації.
- Логування паролів у відкритому вигляді корисне лише для дебагу; перед продакшеном варто очистити.
- Додайте інтеграційні тести, які гоняють основні curl-сценарії через JUnit + `HttpClient`.
