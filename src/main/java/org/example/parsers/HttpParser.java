...
public class HttpParser {
  public static String[] parseHTTP(BufferedReader in, Socket socket) throws IOException {
    ...
    if (authorization != null && authorization.startsWith("Basic ")) {
      try {
        ...
        String userPass =
            new String(
                java.util.Base64.getDecoder().decode(b64), java.nio.charset.StandardCharsets.UTF_8);
  ...
  private static String readFixedLengthBody(BufferedReader in, int length) throws IOException {
    ...
    // Для чистого ASCII/UTF-8 JSON зазвичай збігається. Для 100% точності краще читати з
    // InputStream.
